...
VIDEO_SEARCHPATTERN_SQLITE = "MyVideos??.db"
VIDEO_SEARCHPATTERN_DB = "MyVideos__"
MINVER=75
...
class CDatabase:
    ...
    def getMySQLDBName(self,pattern):
        ...
        if db is None:
            ...
            exit()
        elif int(db[-2:]) < MINVER:
            error("CDatabase: XBMC Databaseversion must be grater than 74")
            exit()
        else:
            return str(db)
